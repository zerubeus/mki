...
import { HISTORICAL_EVENTS, MAP_INITIAL_CENTER, MAP_INITIAL_ZOOM, MAP_EVENT_ZOOM } from '../data/seerahEvents'; // Adjusted path
import type { HistoricalEvent } from '../types'; // Adjusted path

const SeerahJourney: React.FC = () => {
  const [selectedEventId, setSelectedEventId] = useState<number | null>(null);
  const [mapCenter, setMapCenter] = useState<[number, number]>(MAP_INITIAL_CENTER);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-blue-100 flex flex-col p-4 md:p-8 space-y-6">
      <header className="text-center">
        <h1 className="text-3xl md:text-4xl font-bold text-gray-800">
          The Prophet's Path <span style={{fontFamily: "'Amiri Quran', serif"}}>ﷺ</span>
        </h1>
        <p className="text-sm md:text-md text-gray-600 mt-1">
          An Interactive Journey Through Seerah
        </p>
      </header>
